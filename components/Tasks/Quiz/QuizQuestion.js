    ...
    <View>
      ...
      <View style={styles.answers_block}>
        {question.task_answers.map((item, index) => {
          return <FlatButton chooseButton={chooseButton == index} isDisable={isDisable} key={item}
                             onPress={() => onClickVariant(index)} text={item}
                             correct={index == question.task_correct_answer}
          />
        })}
      </View>
    </View>
